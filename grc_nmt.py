#!/usr/bin/env python3
"""
Ancient Greek ⟷ English NMT — end‑to‑end pipeline (mBART/NLLB‑ready)
...
Default model: mBART‑50 many‑to‑many. You can switch to NLLB by changing --model_name and language token codes.
...
python grc_nmt.py make_splits \
  ...
  --split_by_meta data/meta.tsv  # optional: columns id,work,author

# 3) Fine‑tune GRC→EN (mBART default)
...
— Models
  • mBART‑50 (default here) and NLLB‑200 work well; Marian (Helsinki‑NLP/opus‑mt‑*) is also fine with fewer moving parts.
  • For mBART/NLLB you often must set language token codes (see args below). This script helps auto‑guess if not provided.
— Repro tips
...
def norm_greek(text: str, keep_diacritics: bool = True, lowercase: bool = True) -> str:
    ...
    # map final sigma to base sigma in the middle of words
...
def tok_fn_builder(tokenizer, max_source_length: int, max_target_length: int, src_field: str, tgt_field: str):
    def _fn(batch):
        ...
        # Use modern API (no as_target_tokenizer context)
        labels = tokenizer(text_target=batch[tgt_field], max_length=max_target_length, truncation=True)
        model_inputs["labels"] = labels["input_ids"]
        return model_inputs
    return _fn

def guess_lang_codes(model_name: str, direction: str):
    ...
    if "nllb" in name:
        if direction == "grc2en":
            return "ell_Grek", "eng_Latn"
        else:
            return "eng_Latn", "ell_Grek"
    return None, None
...
def cmd_train(args: argparse.Namespace):
    ...
    # Language tokens (mBART/NLLB): best‑effort defaults if not provided
    src_code = args.src_lang_tok_code
    tgt_code = args.tgt_lang_tok_code
    if not src_code or not tgt_code:
        gsrc, gtgt = guess_lang_codes(model_name, args.direction)
        src_code = src_code or gsrc
        tgt_code = tgt_code or gtgt

    if hasattr(tokenizer, "src_lang") and src_code:
        tokenizer.src_lang = src_code
    if hasattr(tokenizer, "tgt_lang") and tgt_code:
        tokenizer.tgt_lang = tgt_code

    # If model requires forced BOS token (e.g., mBART/NLLB), set it for generation during eval
    forced_bos_token_id = None
    target_code_for_bos = args.forced_bos_lang or tgt_code
    if hasattr(tokenizer, "lang_code_to_id") and target_code_for_bos:
        forced_bos_token_id = tokenizer.lang_code_to_id.get(target_code_for_bos)
        if forced_bos_token_id is None:
            print(f"Warning: language code {target_code_for_bos} not in tokenizer.lang_code_to_id")
...
def cmd_generate(args: argparse.Namespace):
    ...
    # Tokenizer language tokens for mBART/NLLB at inference
    forced_bos_token_id = None
    if hasattr(tok, "src_lang") and args.src_lang_tok_code:
        tok.src_lang = args.src_lang_tok_code
    if hasattr(tok, "tgt_lang") and args.tgt_lang_tok_code:
        tok.tgt_lang = args.tgt_lang_tok_code
    if hasattr(tok, "lang_code_to_id"):
        target_code_for_bos = args.forced_bos_lang or args.tgt_lang_tok_code
        if target_code_for_bos:
            forced_bos_token_id = tok.lang_code_to_id.get(target_code_for_bos)
...
def cmd_evaluate(args: argparse.Namespace):
    ...
    # Tokenizer language tokens for evaluation decoding
    forced_bos_token_id = None
    if hasattr(tok, "src_lang") and args.src_lang_tok_code:
        tok.src_lang = args.src_lang_tok_code
    if hasattr(tok, "tgt_lang") and args.tgt_lang_tok_code:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    tr.add_argument("--model_name", default="facebook/mbart-large-50-many-to-many-mmt")
    ...
    tr.add_argument("--forced_bos_lang", default=None, help="e.g., en_XX for mBART or eng_Latn for NLLB; optional")
    tr.add_argument("--src_lang_tok_code", default=None, help="Tokenizer source language code (mBART: el_GR/en_XX; NLLB: ell_Grek/eng_Latn)")
    tr.add_argument("--tgt_lang_tok_code", default=None, help="Tokenizer target language code (mBART: en_XX/el_GR; NLLB: eng_Latn/ell_Grek)")
    ...
    ge.add_argument("--src_lang_tok_code", default=None)
    ge.add_argument("--tgt_lang_tok_code", default=None)
    ge.add_argument("--forced_bos_lang", default=None)
    ...
    ev.add_argument("--src_lang_tok_code", default=None)
    ev.add_argument("--tgt_lang_tok_code", default=None)
    ev.add_argument("--forced_bos_lang", default=None)
